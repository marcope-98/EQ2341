...
class GaussD:
		...
		def logprob(self, x):
			# provided in the announcement section:
			...
			z = np.dot(self.covEigen, (x-np.matlib.repmat(self.means, 1, nx)))

			z /= np.matlib.repmat(np.expand_dims(self.stdevs, 1), 1, nx)

			logP = -np.sum(z*z, axis=0)/2 
			logP = logP - sum(np.log(self.stdevs)) - dSize*np.log(2*np.pi)/2
