...
class DiscreteD:
		...
		def rand(self, nData):
				...
				# R = np.random.choice(@param1, @param2, replace=@param3, p=@param4)
				#			@param1: int,     (converted into np.arange(@param1)), list of number that the function chooses from
				#     @param2: int,      number of random scalars drwan from the Discrete Distribution
				#     @param3: bool,     if true then the sampling is done with replacement (the same value can be sampled multiple times)
				# 		@param4: np.array, the array/list of probabilities to choose an element, in our case the probability mass

				# in our case np.random.choice, chooses a random integer in the range [0,@param1 - 1]
				# repeat the sampling nData times
				# it is allowed to choose the same integer multiple times
				# the choice of the integer is influenced by the discrete distribution 
				
				R = np.random.choice(len(self.probMass), nData, replace=True, p=self.probMass)
		...
		def prob(self, Z):
			n_obs = len(Z)															# number of observation
			#n_states = np.shape(self.probMass)[0]				# number of states
			res = np.empty(shape=(n_obs))			
			for i in np.arange(n_obs):									# for each observation ...
				res[i] = self.probMass[Z[i]-1]				# ... take the column of the self.probMass and appends it to the res matrix
			return res
